...
public class OfferFacadeTest {
	...
	public void should_fetch_from_jobs_from_remote_and_save_all_offers_when_repository_is_empty() {
		// given
		final OfferFacade offerFacade = new OfferFacadeTestConfiguration().offerFacadeForTests();
		assertThat(offerFacade.findAllOffers()).isEmpty();

		// when
		final List<OfferResponseDto> result = offerFacade.fetchAllOffersAndSaveAllIfNotExists();
	...
	public void should_save_only_2_offers_when_repository_had_4_added_with_offer_urls() {
		// given
		final OfferFacade offerFacade = new OfferFacadeTestConfiguration(
				List.of(
		...
		// when
		final List<OfferResponseDto> response = offerFacade.fetchAllOffersAndSaveAllIfNotExists();

		// then
		assertThat(List.of(
						response.get(0).getOfferUrl(),
						response.get(1).getOfferUrl()
				)
	...
	public void should_save_4_offers_when_there_are_no_offers_in_database() {
		// given
		final OfferFacade offerFacade = new OfferFacadeTestConfiguration(List.of()).offerFacadeForTests();
	...
	public void should_find_offer_by_id_when_offer_was_saved() {
		// given
		final OfferFacade offerFacade = new OfferFacadeTestConfiguration(List.of()).offerFacadeForTests();
		final OfferResponseDto offerResponseDto = offerFacade.saveOffer(new OfferRequestDto("id", "asds", "asdasd", "1"));

		// when
		final OfferResponseDto offerById = offerFacade.findOfferById(offerResponseDto.getId());

		// then
		assertThat(offerById).isEqualTo(OfferResponseDto.builder()
				.id(offerResponseDto.getId())
				.companyName("id")
				.position("asds")
				.salary("asdasd")
				.offerUrl("1")
	...
	public void should_throw_not_found_exception_when_offer_not_found() {
		...
		// when
		final Throwable thrown = catchThrowable(() -> offerFacade.findOfferById("100"));
	...
	public void should_throw_duplicate_key_exception_when_with_offer_url_exists() {
		// given
		final OfferFacade offerFacade = new OfferFacadeTestConfiguration(List.of()).offerFacadeForTests();
		final OfferResponseDto offerResponseDto = offerFacade.saveOffer(new OfferRequestDto("id", "asds", "asdasd", "hello.pl"));
		final String savedId = offerResponseDto.getId();
		assertThat(offerFacade.findOfferById(savedId).getId()).isEqualTo(savedId);

		// when
		final Throwable thrown = catchThrowable(() -> offerFacade.saveOffer(
